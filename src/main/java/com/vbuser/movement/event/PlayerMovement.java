    ...
    public void climbMainMethod(TickEvent.PlayerTickEvent event) {
        ...
        if (player.moveForward == 0 && player.moveStrafing == 0) {
            if (climbMap.get(player.getUniqueID())) {
                if (posMap.containsKey(player.getUniqueID()))
                    player.setPosition(posMap.get(player.getUniqueID()).getX(), posMap.get(player.getUniqueID()).getY(), posMap.get(player.getUniqueID()).getZ());
            }
        } else {
