...
public class FengCheJu extends FlowerBase {

    public static final PropertyBool PICKED = PropertyBool.create("picked");

    public FengCheJu(String name, Material material){
        super(name,material);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ){
        if (worldIn.isRemote)
        {
            ...
            if (!state.getValue(PICKED)) {
                EntityItem entityitem = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(Item.getItemFromBlock(ModBlocks.FENG_CHE_JU), 1));
