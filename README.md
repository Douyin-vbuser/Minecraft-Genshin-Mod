...
- [1.2] 制作怪物等实体<sup>[1]</sup>的geckolib模型贴图和动画、AI逻辑
- [2.1] 向com.vbuser.inventory中引入类似原版DataManager的实体数据机制，引入世界数据机制
- [2.2] 向com.vbuser.inventory中引入角色系统
...
- [1.1] 制作客户端启动器，实现启动与MCI更新、下载、文件校验(虽然但是，作为一个开源项目到底得塞多少检测才安全啊)

~~不会有人用比自己年纪大不少的语言(vba)吧~~

### Minecraft地图方向：

- [1.1] 截至1.0开放的蒙德地区地形还原(1.20.1+Axiom，完成地形后保留，转1.12.2版本，不计划通过blockblender读取内存的方法实现以规避潜在的知识产权风险)
- [1.2] 兼容模组实现（方块、生物、方块数据、维度实装、剧情等命令）

### 更好的原神项目方向：

- [1.1] 世界任务的配音补齐
- [1.2] 可视化低代码剧情制作(Web前端或Windows应用)，用于征集现存剧情修订稿